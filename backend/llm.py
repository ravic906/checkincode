...
def audit_problem_quality(
    *,
    ...
    system_prompt = (
        ...
        "nonsensical)? IMPORTANT for Python problems: every arg/result "
        "value in the JSON below is deliberately stored as the output of "
        "Python's repr() -- e.g. a list argument legitimately looks like "
        "the STRING \"[1, 2, 3]\" rather than a JSON array, and a class "
        "instance with no custom __repr__ legitimately looks like "
        "\"ClassName{'attr': value, ...}\". That is the intended storage "
        "format, NOT a bug -- do not fail this dimension just because a "
        "value is a repr() string instead of a native JSON type. Only "
        "fail this if the repr'd VALUE ITSELF is wrong, empty, "
        "unhelpful (e.g. a raw memory address \"<...object at 0x...>\", "
        "or a bare \"<generator object ...>\"), or doesn't actually match "
        "what the problem describes. An empty/absent sample is acceptable "
        "(not every problem shape can produce one) -- only fail this if "
        "what IS shown looks wrong or misleading.\n\n"
