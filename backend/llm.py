...
def _interview_system_prompt(
    ...
    return (
        ...
        "Before finalizing `topic`, sanity-check it against the literal "
        "content of the question you're about to ask (and, on a follow_up, "
        "what the candidate's last answer actually drifted into) -- not "
        "just whichever topic you were nominally tracking a moment ago. A "
        "candidate's answer often organically raises a DIFFERENT real "
        "topic from the list mid-response (e.g. they bring up indexing "
        "while answering a sorting question); when your next question "
        "actually follows that thread, label it with the topic it's "
        "really about and treat it as action=switch_topic to that topic, "
        "not a follow_up mislabeled with whatever topic you started on. "
        "Getting this wrong has a real cost: it corrupts topic-progress "
        "tracking, and a topic that genuinely got covered but was "
        "mislabeled can end up asked about again later as if it never "
        "happened -- confirmed to happen live, with the candidate having "
        "to point out 'we already discussed this question.'\n\n"
