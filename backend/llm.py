...
def audit_problem_quality(
    *,
    ...
    system_prompt = (
        ...
        "5. sample_io_sane -- READ THIS CAREFULLY, every Python example on "
        "this platform is stored this exact way and it is NOT a defect: "
        "every arg/result value is the output of Python's own repr() "
        "function, always a JSON string, NEVER a native JSON array/object. "
        "This is completely normal and expected, e.g. exactly this shape "
        "is a CORRECT, PASSING example, not something to flag:\n"
        '  {"args": ["[{\'customer_id\': 1, \'amount\': 100}, '
        '{\'customer_id\': 2, \'amount\': 200}]"], "result": "{1: 100, 2: 200}"}\n'
        "Notice the args value is a STRING that happens to look like a "
        "list of dicts, and the result is a STRING that happens to look "
        "like a dict -- that is repr() output, always correct, always "
        "expected. Do NOT fail sample_io_sane because a value \"should be "
        "a list/dict instead of a string\", because a string value has "
        "quotes around it, because it \"isn't valid JSON\", or because of "
        "single-vs-double-quote style anywhere inside the repr'd string -- "
        "repr() ALWAYS uses single quotes for strings and NEVER produces "
        "valid JSON, on every correct example on this platform, with no "
        "exceptions. Quote style and JSON-validity must never appear in "
        "your reason for this field. The ONLY real defects to flag "
        "here: the example is completely empty/absent when it plausibly "
        "shouldn't be, the repr'd value is a raw memory address (contains "
        "\"object at 0x\"), the repr'd value is a bare generator "
        "(\"<generator object\"), or -- the one case that actually needs "
        "arithmetic -- the numbers/values shown are factually wrong given "
        "the described behavior (e.g. a claimed sum that doesn't add up). "
        "If none of those four apply, this MUST be ok: true.\n\n"
