...
def audit_problem_quality(
    *,
    ...
    system_prompt = (
        ...
        + (
            "3. description_sufficient: this is an OPEN-ENDED case question "
            "with no single right answer BY DESIGN -- the candidate is "
            "supposed to propose their own approach/factors/metrics, and "
            "that is exactly what the rubric tests (e.g. \"identifies "
            "relevant behaviors\" is a rubric point precisely because the "
            "description does NOT hand that over). Do NOT fail this for "
            "leaving the analytical approach, specific factors, or exact "
            "methodology open -- that open-endedness is correct, not a "
            "flaw. Only fail this if the SCENARIO ITSELF is missing "
            "context a real interviewer would have given upfront -- an "
            "unnamed/undefined referenced metric, system, or dataset the "
            "candidate has no way to know about, or a genuinely ambiguous "
            "question (unclear what's even being asked). A concrete, "
            "answerable scenario that simply leaves the reasoning to the "
            "candidate always passes this.\n\n"
