...
PROBLEM_BATCH_SYSTEM_PROMPT = (
    ...
    "Google, Netflix, and peers like Uber/Airbnb/Stripe) Data Analyst "
    "screens specifically, not a generic bootcamp exercise. That bar "
    "leans heavily on: cohort retention/churn, funnel drop-off between "
    "steps, running/rolling metrics (7-day active users, moving "
    "averages), period-over-period growth, ranking within groups "
    "(top-N per category, nth-highest), sessionization from raw event "
    "logs, and deduplication of noisy real-world data -- draw hard "
    "problems from this pool rather than inventing artificial syntax "
    "puzzles. If you wouldn't expect an actual FAANG interviewer to "
    "plausibly ask a version of this question, don't include it.\n\n"
...
PYTHON_PROBLEM_BATCH_SYSTEM_PROMPT = (
    ...
    "Calibrate to the bar set by FAANG-caliber (Meta, Amazon, Apple, "
    "Google, Netflix, and peers like Uber/Airbnb/Stripe) Data Analyst "
    "coding screens specifically -- these lean toward practical data-"
    "wrangling and correctness-under-edge-cases (parsing/cleaning messy "
    "records, grouping and aggregating nested structures, merging "
    "overlapping intervals, deduplicating near-identical entries, "
    "efficient counting/frequency problems) rather than pure LeetCode-"
    "style algorithm-contest puzzles (no need for advanced graph "
    "algorithms, dynamic programming on exotic state spaces, or "
    "competitive-programming-only tricks). If you wouldn't expect an "
    "actual FAANG interviewer to plausibly ask a version of this "
    "question, don't include it.\n\n"
    "Every `description` MUST be fully unambiguous about WHAT is being "
...
STATS_PYTHON_BATCH_SYSTEM_PROMPT = (
    ...
    "Calibrate to the bar set by FAANG-caliber (Meta, Amazon, Apple, "
    "Google, Netflix, and peers like Uber/Airbnb/Stripe) Data Analyst/"
    "Data Scientist experimentation rounds specifically -- these lean "
    "heavily on A/B test read-outs (is this lift real, is the sample "
    "size adequate, is novelty effect a risk), metric-movement root-"
    "causing, guardrail-metric trade-offs, and correctly reasoning about "
    "statistical power and multiple-comparison corrections, not just "
    "textbook formula recall. If you wouldn't expect an actual FAANG "
    "interviewer to plausibly ask a version of this question, don't "
    "include it.\n\n"
